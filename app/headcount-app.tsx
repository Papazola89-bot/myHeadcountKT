...
export default function HeadcountApp(){
  ...
  const saveStudent=async(student:Student)=>{
    ...
    if(!appsScriptService){announce(localOnlyMessage);return}
    try{
      const persisted=await appsScriptService.saveStudent({name:student.name,year:student.year,className:student.className,subject:student.subject,status:student.status,startDate:student.startDate});
      setStudents(current=>current.map(item=>item.id===student.id?{...item,id:persisted.id}:item));
