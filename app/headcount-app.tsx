...
export default function HeadcountApp(){
  ...
  const [year,setYear]=useState("Semua tahun"),[query,setQuery]=useState(""),[selected,setSelected]=useState<Student|null>(null),[modal,setModal]=useState<"add"|"intervention"|"submit"|"profile"|"admin"|null>(null);
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      if(!appsScriptService){
        if(active){setStudents([]);setSchools([]);setAdmins([]);setInterventions([]);setAdminInterventions([]);setSheetStatus("idle");setSaved("Log masuk untuk menyambung Google Sheets")}
        return;
      }
      ...
      try{
        ...
        if(safeProfile.role==="ADMIN"){
          ...
          if(active){setSchools(remoteSchools);setAdminInterventions(remoteInterventions);setAdmins(remoteAdmins)}
        }else{
        ...
        if(!active)return;
        setStudents([]);setSchools([]);setAdmins([]);setInterventions([]);setAdminInterventions([]);setSchoolsLoading(false);setInterventionsLoading(false);
  ...
  const signOut=()=>{window.google?.accounts.id.disableAutoSelect();setIdToken("");setSchoolSessionToken("");setAuthMethod(null);setGoogleEmail("");setProfile(null);setStudents([]);setSchools([]);setAdmins([]);setInterventions([]);setAdminInterventions([]);setAuthStatus("signed-out");setSheetStatus("idle");setSaved("Log masuk untuk menyambung Sheets");setRole("guru");setView("dashboard");setSelected(null);setModal(null);setMobile(false);setNotificationsOpen(false);setNotificationsRead(false);announce("Anda telah log keluar daripada sesi myHeadcountKT.")};
