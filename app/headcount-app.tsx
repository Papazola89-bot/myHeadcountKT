...
import {
  ...
  LogOut, Menu, MoreHorizontal, Pencil, Plus, Printer, RotateCcw, School, Search,
...
import { parseAnalysisExport, type AnalysisImportPreview } from "./lib/analysis-import";
...
type Intervention = { id:string; studentId:string; issue:string; action:string; method:string; start:string; review:string; status:string; skillCode:string; skillName:string; notes:string; groupId:string };
type InterventionDraft = { mode:"individual"|"group"; targetId:string; skillCode:string; skillName:string; issue:string; action:string; notes:string; method:string; start:string; review:string; status:string };
...
const appsScriptEndpoint=(process.env.NEXT_PUBLIC_DATA_API_URL||"https://jqhkeuiztgfmmwpopxpv.supabase.co/functions/v1/myheadcountkt-api").trim();
...
const interventionFromRecord=(row:InterventionRecord):Intervention=>({id:row.id,studentId:row.studentId,issue:row.issue,action:row.action,method:row.method,start:row.startDate,review:row.reviewDate,status:row.status,skillCode:row.skillCode,skillName:row.skillName,notes:row.notes,groupId:row.groupId});
...
export default function HeadcountApp(){
  ...
  const [interventions,setInterventions]=useState<Intervention[]>([]),[interventionGroups,setInterventionGroups]=useState<InterventionGroupRecord[]>([]),[adminInterventions,setAdminInterventions]=useState<InterventionRecord[]>([]),[interventionsLoading,setInterventionsLoading]=useState(false),[cycle,setCycle]=useState<Cycle>("AR 3"),[subject,setSubject]=useState<Subject|"Semua">("Bahasa Melayu");
  const [year,setYear]=useState("Semua tahun"),[query,setQuery]=useState(""),[selected,setSelected]=useState<Student|null>(null),[editCandidate,setEditCandidate]=useState<Student|null>(null),[deleteCandidate,setDeleteCandidate]=useState<Student|null>(null),[editingGroup,setEditingGroup]=useState<InterventionGroupRecord|null>(null),[recordGroupId,setRecordGroupId]=useState(""),[modal,setModal]=useState<"add"|"analysis-import"|"edit-student"|"delete-student"|"intervention"|"group"|"submit"|"profile"|"admin"|"transfer"|"import"|null>(null);
  const [mobile,setMobile]=useState(false),[toast,setToast]=useState(""),[saved,setSaved]=useState("Menunggu sambungan Supabase"),[undo,setUndo]=useState<Student[]|null>(null);
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      if(!appsScriptService){
        if(active){setStudents([]);setSchools([]);setAdmins([]);setSchoolDirectory([]);setTransfers([]);setInterventions([]);setInterventionGroups([]);setAdminInterventions([]);setSyncNotice(null);setSheetStatus("idle");setSaved("Log masuk untuk menyambung Supabase")}
        return;
      }
      setSheetStatus("connecting");setSaved("Menyambung ke Supabase...");
      try{
        ...
        if(safeProfile.role==="ADMIN"){
          ...
          const [remoteInterventions,remoteGroups,remoteDirectory,remoteTransfers]=await Promise.all([appsScriptService.getInterventions(),appsScriptService.getInterventionGroups().catch(()=>[]),appsScriptService.getSchoolDirectory(),appsScriptService.getTransfers()]);
          if(active){setInterventions(remoteInterventions.map(interventionFromRecord));setInterventionGroups(remoteGroups);setSchoolDirectory(remoteDirectory);setTransfers(remoteTransfers)}
        }
        ...
        if(active){
          ...
          setSheetStatus("connected");setSaved("Supabase disambungkan");setSyncNotice({tone:"success",title:"Data berjaya diselaraskan",message:`Sambungan untuk ${identity} telah siap. Data terkini sudah dimuatkan dan portal selamat digunakan.`});
        }
      }catch(error){
        if(!active)return;
        setStudents([]);setSchools([]);setAdmins([]);setSchoolDirectory([]);setTransfers([]);setInterventions([]);setInterventionGroups([]);setAdminInterventions([]);setSchoolsLoading(false);setInterventionsLoading(false);
        setSheetStatus("fallback");setSaved("Supabase gagal disambungkan");
        const message=error instanceof Error?error.message:"Sambungan tidak berjaya.";
        setSyncNotice({tone:"error",title:"Penyelarasan gagal",message:`Supabase belum dapat disambungkan. Tiada data kosong atau lama dipaparkan. ${message}`.trim()});
        announce(`Supabase tidak dapat dicapai. Tiada data tempatan dipaparkan. ${message}`.trim());
      }
    };
    void load();
    return()=>{active=false};
  },[appsScriptService,authStatus,authMethod,syncAttempt]);
  const retrySync=()=>{setSyncNotice(null);setSheetStatus("connecting");setSaved("Menyambung semula ke Supabase...");setSyncAttempt(attempt=>attempt+1)};
  const signOut=()=>{window.google?.accounts.id.disableAutoSelect();setIdToken("");setSchoolSessionToken("");setAuthMethod(null);setGoogleEmail("");setProfile(null);setStudents([]);setSchools([]);setAdmins([]);setSchoolDirectory([]);setTransfers([]);setInterventions([]);setInterventionGroups([]);setAdminInterventions([]);setSyncNotice(null);setAuthStatus("signed-out");setSheetStatus("idle");setSaved("Log masuk untuk menyambung Supabase");setRole("guru");setView("dashboard");setSelected(null);setEditCandidate(null);setDeleteCandidate(null);setEditingGroup(null);setRecordGroupId("");setModal(null);setMobile(false);setNotificationsOpen(false);setNotificationsRead(false);announce("Anda telah log keluar daripada sesi myHeadcountKT.")};
  ...
  const persist=(next:Student[],previous?:Student[])=>{if(previous)setUndo(previous);setStudents(next);setSaved("Perubahan belum disahkan oleh Supabase")};
  const localOnlyMessage=!(idToken||schoolSessionToken)?"Log masuk untuk menyimpan ke Supabase.":"Supabase belum tersedia. Perubahan belum disimpan.";
  ...
  const updateSkill=async(id:string,value:number)=>{
    ...
    try{
      ...
      setSheetStatus("connected");setSaved(`Supabase · ${new Intl.DateTimeFormat("ms-MY",{hour:"numeric",minute:"2-digit"}).format(new Date())}`);
      ...
      setSheetStatus("fallback");setSaved("Disimpan lokal · Supabase gagal");
  ...
  const saveIntervention=async(draft:InterventionDraft,requestId:string)=>{
    if(!appsScriptService)throw new Error(localOnlyMessage);
    const group=draft.mode==="group"?interventionGroups.find(item=>item.id===draft.targetId):null;
    const studentIds=draft.mode==="individual"?[draft.targetId]:undefined;
    const result=await appsScriptService.saveInterventionBatch({
      ...(group?{groupId:group.id}:{studentIds}),skillCode:draft.skillCode,skillName:draft.skillName,
      issue:draft.issue,intervensi:draft.action,notes:draft.notes,method:draft.method,startDate:draft.start,reviewDate:draft.review,status:draft.status,
    },requestId);
    setInterventions(current=>{const known=new Set(current.map(item=>item.id));return [...result.records.filter(row=>!known.has(row.id)).map(interventionFromRecord),...current]});
    setSaved(`Supabase · ${new Intl.DateTimeFormat("ms-MY",{hour:"numeric",minute:"2-digit"}).format(new Date())}`);
    setModal(null);setSelected(null);setRecordGroupId("");
    announce(result.count>1?`Intervensi berjaya direkodkan kepada ${result.count} orang murid.`:"Intervensi berjaya disimpan ke Supabase.");
    return result.count;
  };
  const saveInterventionGroup=async(payload:{groupId?:string;groupName:string;skillCode:string;skillCodes:string[];skillName:string;studentIds:string[]})=>{
    if(!appsScriptService)throw new Error(localOnlyMessage);
    const savedGroup=await appsScriptService.saveInterventionGroup(payload);
    setInterventionGroups(current=>[savedGroup,...current.filter(item=>item.id!==savedGroup.id)]);
    announce(payload.groupId?"Kumpulan intervensi berjaya dikemas kini.":"Kumpulan intervensi berjaya dicipta.");
    return savedGroup;
  };
  const deleteInterventionGroup=async(group:InterventionGroupRecord)=>{
    if(!appsScriptService)throw new Error(localOnlyMessage);
    if(!window.confirm(`Padam ${group.name}? Rekod intervensi lama setiap murid akan dikekalkan.`))return;
    try{await appsScriptService.deleteInterventionGroup(group.id);setInterventionGroups(current=>current.filter(item=>item.id!==group.id));announce("Kumpulan dipadam. Sejarah intervensi murid dikekalkan.")}
    catch(error){announce(`Kumpulan tidak dapat dipadam. ${error instanceof Error?error.message:""}`.trim())}
  };
  const saveStudent=async(intake:StudentIntake)=>{
    ...
    try{
      ...
      setSheetStatus("connected");announce(intake.subjects.length===2?"Murid berjaya didaftarkan untuk Bahasa Melayu dan Matematik.":"Murid baharu berjaya disimpan ke Supabase.");
    }catch(error){
      setStudents(previous);
  ...
  const importAnalysis=async(preview:AnalysisImportPreview,screening:1|2|3)=>{
    if(!appsScriptService)throw new Error(localOnlyMessage);
    try{
      const result=await appsScriptService.importAnalysis({rows:preview.rows,cycle:`AR ${screening}`,tahun_data:2026});
      setStudents(result.students);setCycle(`AR ${screening}` as Cycle);setModal(null);setSheetStatus("connected");setSaved(`${result.recordCount} kemahiran disimpan ke Supabase`);
      announce(`${result.pupilCount} murid dikesan. Nama, darjah dan ${result.recordCount} kemahiran diisi automatik untuk Saringan ${screening}.`);
    }catch(error){announce(`Import tidak berjaya. ${error instanceof Error?error.message:""}`.trim());throw error}
  };
  const deleteStudent=async(student:Student,confirmation:string)=>{
    if(!appsScriptService)throw new Error(localOnlyMessage);
    try{
      const result=await appsScriptService.deleteStudent(student.id,confirmation);
      const ids=new Set(result.deletedIds);setStudents(current=>current.filter(item=>!ids.has(item.id)));setSelected(null);setDeleteCandidate(null);setModal(null);setSheetStatus("connected");
      announce(`${student.name} dan semua rekod subjeknya telah dipadam daripada Supabase.`);
    }catch(error){announce(`Murid tidak dapat dipadam. ${error instanceof Error?error.message:""}`.trim());throw error}
  };
  const updateStudent=async(payload:{studentId:string;name:string;year:number;className:string;startDate:string;status:Student["status"];groupId:string})=>{
    if(!appsScriptService)throw new Error(localOnlyMessage);
    try{
      const result=await appsScriptService.updateStudent(payload);setStudents(result.students);setInterventionGroups(result.groups);setSelected(null);setEditCandidate(null);setModal(null);setSheetStatus("connected");announce(`${payload.name} berjaya dikemas kini dan kumpulan intervensi telah diselaraskan.`);
    }catch(error){announce(`Murid tidak dapat dikemas kini. ${error instanceof Error?error.message:""}`.trim());throw error}
  };
  const submitCurrentCycle=async()=>{
    if(!appsScriptService){setModal(null);announce(!(idToken||schoolSessionToken)?"Log masuk dahulu untuk menghantar data kepada admin.":"Supabase belum tersedia; data kekal sebagai draf.");return}
    try{
      await appsScriptService.submitCycle(cycle,{subject:subject==="Semua"?"Bahasa Melayu":subject,tahun:2026});
      setSheetStatus("connected");setSubmission(current=>({...current,[cycle]:"Telah Dihantar"}));setModal(null);announce(`Data ${cycle} telah dihantar dan direkodkan dalam Supabase.`);
    }catch(error){
  ...
  const saveProfileName=async(name:string)=>{
    ...
    try{
      ...
      setProfile(updated);setModal(null);announce("Profil berjaya dikemas kini dalam Supabase.");
    }catch(error){
  ...
  const addAdmin=async(payload:{email:string;name:string})=>{
    if(!appsScriptService)throw new Error("Supabase belum tersedia.");
    try{
  ...
  const addSchool=async(payload:{code:string;name:string;zone:string})=>{
    ...
    try{
      ...
      announce(`${created.name} berjaya ditambah ke Supabase.`);
      return created;
  ...
  const clearAllData=async(confirmation:string)=>{
    ...
    try{
      ...
      setSaved("Data operasi Supabase telah dikosongkan");
  ...
  const deleteSchool=async(school:SchoolRecord)=>{
    if(!appsScriptService){announce("Supabase belum tersedia.");return}
    try{
      await appsScriptService.deleteSchool(school.id);
      setSchools(current=>current.filter(item=>item.id!==school.id));
      announce(`${school.name} telah dipadam daripada Supabase.`);
  ...
  const clearSchools=async(confirmation:string)=>{
    if(!appsScriptService){announce("Supabase belum tersedia.");return}
    try{
      await appsScriptService.clearSchools(confirmation);setSchools([]);announce("Semua rekod sekolah kosong telah dipadam daripada Supabase.");
  ...
  const sheetLabel=sheetStatus==="connected"?"Supabase disambungkan":sheetStatus==="connecting"?"Menyambung Supabase":sheetStatus==="fallback"?"Supabase gagal":"Supabase belum disambungkan";
  ...
  if(sheetStatus==="fallback")return <SyncResultScreen notice={syncNotice||{tone:"error",title:"Penyelarasan gagal",message:"Supabase belum dapat disambungkan. Tiada data dipaparkan sehingga sambungan berjaya."}} onRetry={retrySync} onLogout={signOut}/>;
  ...
  return <div className="app-shell">
    ...
    <main><header className="topbar"><div><button className="menu-btn" onClick={()=>setMobile(true)}><Menu size={21}/></button><span><small>{role==="guru"?"PORTAL GURU":"PORTAL ADMIN"}</small><strong>{menu.find(x=>x[0]===view)?.[1]||"Tetapan"}</strong></span></div><div><div className={`google-session ${authStatus} ${sheetStatus}`}><span className="connection-state"><i/><b>{authMethod==="school"?"Kod sekolah disahkan":"Google admin disahkan"}</b><small>{authMethod==="school"?`${profile.schoolCode} · ${sheetLabel}`:`${googleEmail} · ${sheetLabel}`}</small></span><button className="google-signout" onClick={signOut} title="Log keluar"><LogOut size={15}/>Log keluar</button></div><button className={"bell "+(notificationsOpen?"active":"")} onClick={()=>setNotificationsOpen(open=>!open)} aria-label={notificationsOpen?"Tutup notifikasi":"Buka notifikasi"} aria-expanded={notificationsOpen}><Bell size={20}/>{notifications.length>0&&!notificationsRead&&<i/>}</button>{notificationsOpen&&<><button className="notification-scrim" onClick={()=>setNotificationsOpen(false)} aria-label="Tutup notifikasi"/><aside className="notifications-panel"><header><span><strong>Notifikasi</strong><small>{notifications.length?notificationsRead?"Semua telah dibaca":notifications.length+" notifikasi baharu":"Tiada notifikasi"}</small></span><button onClick={()=>setNotificationsRead(true)} disabled={notificationsRead||!notifications.length}>Tandakan semua dibaca</button></header><div>{notifications.length?notifications.map(({title,detail,time,view:target,Icon,tone})=><button className="notification-item" key={title} onClick={()=>{setNotificationsRead(true);go(target)}}><i className={tone}><Icon size={17}/></i><span><strong>{title}</strong><small>{detail}</small><em>{time}</em></span>{!notificationsRead&&<b/>}</button>):<div className="empty-state"><strong>Tiada notifikasi</strong><p>Notifikasi contoh telah dibuang.</p></div>}</div><footer><ShieldCheck size={14}/>Notifikasi sistem myHeadcountKT</footer></aside></>}<button className="profile" onClick={()=>go("settings")}><b className="avatar">{userInitials}</b><span>{userName}</span><ChevronDown size={15}/></button></div></header>
      <div className="page">
        {role==="guru"?<>
          {view==="dashboard"&&<GuruDashboard {...props}/>} {view==="students"&&<StudentsView {...props} pendingTransfers={transfers.filter(record=>record.status.toLowerCase()==="menunggu import"&&record.toSchoolId===profile.schoolId)} onAdd={()=>setModal("add")} onImport={()=>setModal("import")} onAnalysisImport={()=>setModal("analysis-import")} onIntervention={(s:Student)=>{setSelected(s);setModal("intervention")}} onEdit={(s:Student)=>{setEditCandidate(s);setModal("edit-student")}} onDelete={(s:Student)=>{setDeleteCandidate(s);setModal("delete-student")}}/>}
          ...
          {view==="interventions"&&<Interventions students={students} interventions={interventions} groups={interventionGroups} setSelected={setSelected} onAddIndividual={()=>{setRecordGroupId("");setModal("intervention")}} onAddGroup={()=>{setEditingGroup(null);setModal("group")}} onEditGroup={group=>{setEditingGroup(group);setModal("group")}} onRecordGroup={group=>{setRecordGroupId(group.id);setModal("intervention")}} onDeleteGroup={group=>{void deleteInterventionGroup(group)}} announce={announce}/>}
          {view==="analysis"&&<Analysis students={students} exportCsv={exportCsv}/>}
          {view==="reports"&&<Reports role={role} hasData={students.length>0} exportCsv={exportCsv} announce={announce}/>}
          {view==="settings"&&<SettingsView profile={profile} onEdit={()=>profile?setModal("profile"):announce("Log masuk untuk mengemas kini profil.")} clearAllData={clearAllData}/>}
        </>:<>
    ...
    {selected&&!modal&&<StudentDrawer student={selected} close={()=>setSelected(null)} intervention={()=>setModal("intervention")} transfer={()=>setModal("transfer")} edit={()=>{setEditCandidate(selected);setModal("edit-student")}} remove={()=>{setDeleteCandidate(selected);setModal("delete-student")}}/>}
    {modal==="add"&&<AddModal close={()=>setModal(null)} save={saveStudent}/>}
    {modal==="analysis-import"&&<AnalysisImportModal close={()=>setModal(null)} save={importAnalysis}/>}
    {modal==="edit-student"&&editCandidate&&<EditStudentModal student={editCandidate} groups={interventionGroups} close={()=>{setModal(null);setEditCandidate(null)}} save={updateStudent}/>}
    {modal==="delete-student"&&deleteCandidate&&<DeleteStudentModal student={deleteCandidate} close={()=>{setModal(null);setDeleteCandidate(null)}} confirm={text=>deleteStudent(deleteCandidate,text)}/>}
    {modal==="intervention"&&<InterventionModal students={students} groups={interventionGroups} selected={selected} initialGroupId={recordGroupId} close={()=>{setModal(null);setSelected(null);setRecordGroupId("")}} save={saveIntervention}/>}
    {modal==="group"&&<InterventionGroupModal students={students} groups={interventionGroups} group={editingGroup} close={()=>{setModal(null);setEditingGroup(null)}} save={saveInterventionGroup}/>}
...
function SyncLoadingScreen({profile,authMethod}:{profile:UserProfile|null;authMethod:AuthMethod}){
  ...
  return <main className="sync-page" aria-busy="true" aria-live="polite">
    <section className="sync-card">
      ...
      <h1>Menyelaraskan data Supabase</h1>
      ...
      <ol className="sync-steps">
        ...
        <li className="active"><i/><span><strong>Menyambung Supabase</strong><small>Memeriksa sambungan pangkalan data</small></span></li>
  ...
  return <main className="login-page">
    <section className="login-showcase">
      ...
      <div className="login-features">
        <article><i><FileSpreadsheet size={20}/></i><span><strong>Rekod berpusat</strong><small>Data disimpan terus ke Supabase</small></span></article>
...
function StudentsView(p:CommonProps&{pendingTransfers:TransferRecord[];onAdd:()=>void;onImport:()=>void;onAnalysisImport:()=>void;onIntervention:(s:Student)=>void;onEdit:(s:Student)=>void;onDelete:(s:Student)=>void}){const downloadTemplate=()=>{downloadCsvFile("template-murid-myHeadcountKT.csv",[["ID","Nama","Tahun","Kelas","Subjek","Status","Tarikh Mula"]]);p.announce("Template murid telah dimuat turun.")};return <>
...
function Headcount(p:CommonProps&{saved:string;updateSkill:(id:string,v:number)=>void;toggleManualOti:(id:string,enabled:boolean)=>void;undo:()=>void;canUndo:boolean;submission:Record<string,string>;onSubmit:()=>void}){
  ...
  return <><Heading eyebrow="REKOD PENILAIAN" title="Headcount Murid" desc="Kemas kini kemahiran murid dengan pantas. Setiap perubahan disimpan automatik."><div className="heading-actions"><span className="saved"><Check size={15}/>{p.saved}</span><button className="outline" disabled={!p.canUndo} onClick={p.undo}><RotateCcw size={16}/>Undo</button><button className="primary" disabled={locked} onClick={p.onSubmit}><Upload size={17}/>Hantar Data</button></div></Heading>
    ...
    <section className="card sheet">
      ...
      <footer className="autosave"><span><CheckCircle2 size={16}/>Semua perubahan disimpan ke Supabase</span><span>OTI hanya dikira semula apabila TOV atau ETR berubah</span></footer>
    </section>
  </>}

function Interventions({students,interventions,groups,setSelected,onAddIndividual,onAddGroup,onEditGroup,onRecordGroup,onDeleteGroup,announce}:{students:Student[];interventions:Intervention[];groups:InterventionGroupRecord[];setSelected:(s:Student)=>void;onAddIndividual:()=>void;onAddGroup:()=>void;onEditGroup:(group:InterventionGroupRecord)=>void;onRecordGroup:(group:InterventionGroupRecord)=>void;onDeleteGroup:(group:InterventionGroupRecord)=>void;announce:(message:string)=>void}){
  const [mode,setMode]=useState<"individual"|"group">("individual"),[expanded,setExpanded]=useState("");
  const visible=interventions.filter(item=>students.some(student=>student.id===item.studentId));
  const now=new Date(),active=visible.filter(item=>item.status.toLowerCase().includes("sedang")).length,overdue=visible.filter(item=>new Date(item.review)<now&&!/selesai|berjaya/i.test(item.status)).length,successful=visible.filter(item=>/selesai|berjaya/i.test(item.status)).length,followUp=visible.filter(item=>/lanjutan|susulan/i.test(item.status)).length;
  return <><Heading eyebrow="TINDAKAN SUSULAN" title="Intervensi" desc="Rancang intervensi individu atau guna semula kumpulan murid yang mempunyai keperluan sama."><button className="primary" onClick={mode==="group"?onAddGroup:onAddIndividual} disabled={!students.length}><Plus size={18}/>{mode==="group"?"Cipta Kumpulan":"Rekod Intervensi"}</button></Heading>
    <div className="intervention-tabs" role="tablist" aria-label="Jenis intervensi"><button role="tab" aria-selected={mode==="individual"} className={mode==="individual"?"active":""} onClick={()=>setMode("individual")}>Intervensi Individu</button><button role="tab" aria-selected={mode==="group"} className={mode==="group"?"active":""} onClick={()=>setMode("group")}>Intervensi Berkumpulan</button></div>
    {mode==="individual"?<><section className="stat-grid four"><Stat label="Sedang Dilaksana" value={active} detail="Intervensi aktif" Icon={Activity} tone="blue"/><Stat label="Perlu Disemak" value={overdue} detail="Melepasi tarikh semakan" Icon={Clock3} tone="amber"/><Stat label="Berjaya" value={successful} detail="Sasaran dicapai" Icon={CheckCircle2} tone="green"/><Stat label="Perlu Susulan" value={followUp} detail="Strategi baharu" Icon={AlertCircle} tone="red"/></section><section className="card intervention-list"><Toolbar><button className="outline" onClick={()=>announce("Semua status intervensi sedang dipaparkan.")}><Filter size={16}/>Semua status</button><button className="outline" onClick={()=>announce("Intervensi disusun mengikut tarikh semakan.")}><CalendarDays size={16}/>Tarikh semakan</button></Toolbar>{visible.length?visible.map(item=>{const s=students.find(x=>x.id===item.studentId);if(!s)return null;const isOverdue=new Date(item.review)<now&&!/selesai|berjaya/i.test(item.status);return <article key={item.id} className={isOverdue?"urgent":""}><i/><StudentCell s={s} onClick={()=>setSelected(s)}/><div><small>{item.skillCode||"KEMAHIRAN"} · ISU</small><strong>{item.issue}</strong><p>{item.action}</p></div><div><small>KAEDAH</small><strong>{item.method}</strong><em>Mula {date(item.start)}</em></div><div className={isOverdue?"overdue":""}><small>TARIKH SEMAKAN</small><strong><CalendarDays size={15}/>{date(item.review)}</strong>{isOverdue&&<em>Lewat disemak</em>}</div><div><Status text={item.status==="Sedang dilaksanakan"?"Intervensi aktif":item.status}/><button className="outline small" onClick={()=>setSelected(s)}>Semak <ChevronRight size={15}/></button></div></article>}):<div className="empty-state"><i><Activity size={24}/></i><strong>Belum ada intervensi individu</strong><p>Lengkapkan borang dan tekan Simpan sekali untuk merekod ke Supabase.</p></div>}</section></>:<section className="group-grid">{groups.length?groups.map(group=>{const members=group.studentIds.map(id=>students.find(student=>student.id===id)).filter((student):student is Student=>Boolean(student));return <article className="card group-card" key={group.id}><header><i><Users size={21}/></i><span><strong>{group.name}</strong><small>{members.length} Murid</small></span></header><p><b>{group.skillCodes.join(" + ")||group.skillCode}</b>{group.skillName?` – ${group.skillName}`:""}</p><div className="group-actions"><button className="outline small" onClick={()=>setExpanded(expanded===group.id?"":group.id)}>{expanded===group.id?"Tutup Ahli":"Lihat Ahli"}</button><button className="outline small" onClick={()=>onEditGroup(group)}>Edit</button><button className="primary small" onClick={()=>onRecordGroup(group)}>Rekod Intervensi</button><button className="icon danger-icon" onClick={()=>onDeleteGroup(group)} aria-label={`Padam ${group.name}`}><Trash2 size={16}/></button></div>{expanded===group.id&&<ul className="group-members">{members.map(member=><li key={member.id}><b>{initials(member.name)}</b><span>{member.name}<small>{member.className} · {member.subject}</small></span></li>)}</ul>}</article>}):<article className="card empty-state"><i><Users size={24}/></i><strong>Belum ada kumpulan intervensi</strong><p>Cipta kumpulan sekali, kemudian pilih kumpulan itu setiap kali merekod intervensi.</p><button className="primary" onClick={onAddGroup} disabled={!students.length}><Plus size={17}/>Cipta Kumpulan</button></article>}</section>}
  </>;
}
...
function EmptySchools(){return <div className="empty-state"><i><Building2 size={24}/></i><strong>Belum ada sekolah dalam Supabase</strong><p>Tambah rekod pertama di sini atau isi tab SEKOLAH. Data contoh lama tidak lagi dipaparkan.</p></div>}
...
function Audit(){return <><Heading eyebrow="JEJAK SISTEM" title="Audit Log" desc="Rekod perubahan penting dalam Supabase."/><section className="card"><div className="empty-state"><i><History size={24}/></i><strong>Tiada log dipaparkan</strong><p>Log contoh telah dibuang. Tab AUDIT_LOG akan mula merekod tindakan sebenar selepas backend baharu digunakan.</p></div></section></>}
...
function AnalysisImportModal({close,save}:{close:()=>void;save:(preview:AnalysisImportPreview,screening:1|2|3)=>Promise<void>}){
  const [preview,setPreview]=useState<AnalysisImportPreview|null>(null),[screening,setScreening]=useState<1|2|3>(1),[fileName,setFileName]=useState(""),[error,setError]=useState(""),[saving,setSaving]=useState(false);
  const chooseFile=async(file?:File)=>{
    if(!file)return;
    setError("");setPreview(null);setFileName(file.name);
    try{setPreview(parseAnalysisExport(await file.text()))}
    catch(reason){setError(reason instanceof Error?reason.message:"Fail tidak dapat dibaca.")}
  };
  const submit=async()=>{if(!preview||saving)return;setSaving(true);try{await save(preview,screening)}catch{}finally{setSaving(false)}};
  return <Modal title="Auto Isi Senarai Murid" desc="Muat naik fail Analisis Keseluruhan. Sistem mengesan nama, darjah dan kemahiran secara automatik." close={close} footer={<><button className="outline" onClick={close} disabled={saving}>Batal</button><button className="primary" disabled={!preview||saving} onClick={()=>void submit()}><WandSparkles size={17}/>{saving?"Menyimpan...":"Isi & Simpan"}</button></>}>
    <div className="analysis-import">
      <label className="analysis-upload"><Upload size={24}/><span><strong>{fileName||"Pilih fail Analisis Keseluruhan"}</strong><small>Format eksport .xls berasaskan Excel/HTML</small></span><input type="file" accept=".xls,.html" onChange={event=>void chooseFile(event.target.files?.[0])}/></label>
      {error&&<p className="import-error"><AlertCircle size={16}/>{error}</p>}
      {preview&&<>
        <section className="import-summary"><span><small>SEKOLAH DIKESAN</small><strong>{preview.schoolName||"Tidak dinyatakan"}</strong></span><div><b>{preview.pupilCount}<small>Murid</small></b><b>{preview.rows.length}<small>Rekod kemahiran</small></b></div></section>
        <label className="screening-choice"><span><strong>Pilih saringan</strong><small>Ini sahaja pilihan manual yang diperlukan.</small></span><select value={screening} onChange={event=>setScreening(Number(event.target.value) as 1|2|3)}><option value={1}>Saringan 1</option><option value={2}>Saringan 2</option><option value={3}>Saringan 3</option></select></label>
        <div className="import-preview"><header><span>Nama murid</span><span>Darjah</span><span>Kemahiran</span></header>{preview.rows.slice(0,8).map((row,index)=><div key={`${row.name}-${row.subject}-${index}`}><span><strong>{row.name}</strong><small>{row.subject}</small></span><span>Tahun {row.year}</span><b>{row.skillCode}</b></div>)}{preview.rows.length>8&&<footer>+ {preview.rows.length-8} rekod lagi akan diisi secara automatik</footer>}</div>
      </>}
    </div>
  </Modal>
}
...
function EditStudentModal({student,groups,close,save}:{student:Student;groups:InterventionGroupRecord[];close:()=>void;save:(payload:{studentId:string;name:string;year:number;className:string;startDate:string;status:Student["status"];groupId:string})=>Promise<void>}){
  const currentGroup=groups.find(group=>group.studentIds.includes(student.id));
  const [name,setName]=useState(student.name),[year,setYear]=useState(student.year),[className,setClassName]=useState(student.className),[startDate,setStartDate]=useState(student.startDate||todayIso()),[status,setStatus]=useState<Student["status"]>(student.status),[groupId,setGroupId]=useState(currentGroup?.id||""),[saving,setSaving]=useState(false),[error,setError]=useState("");
  const valid=name.trim().length>=2&&className.trim().length>=1;
  const submit=async()=>{if(!valid||saving)return;setSaving(true);setError("");try{await save({studentId:student.id,name:name.trim(),year,className:className.trim(),startDate,status,groupId})}catch(reason){setError(reason instanceof Error?reason.message:"Kemas kini tidak berjaya.")}finally{setSaving(false)}};
  return <Modal title="Edit Murid" desc="Kemas kini profil dan tetapkan kumpulan intervensi tanpa mencipta rekod murid baharu." close={close} footer={<><button className="outline" onClick={close} disabled={saving}>Batal</button><button className="primary" onClick={()=>void submit()} disabled={!valid||saving}><Check size={17}/>{saving?"Menyimpan...":"Simpan Perubahan"}</button></>}><div className="form-grid"><label className="full">Nama murid<input autoFocus value={name} maxLength={300} onChange={event=>setName(event.target.value)} disabled={saving}/></label><label>Darjah<select value={year} onChange={event=>setYear(Number(event.target.value))} disabled={saving}>{STUDENT_YEARS.map(value=><option key={value} value={value}>Tahun {value}</option>)}</select></label><label>Kelas<input value={className} maxLength={100} onChange={event=>setClassName(event.target.value)} disabled={saving}/></label><label>Mata pelajaran<input value={student.subject} readOnly/></label><label>Status<select value={status} onChange={event=>setStatus(event.target.value as Student["status"])} disabled={saving}><option>Aktif</option><option>Pelepasan</option></select></label><label className="full">Tarikh mula Pemulihan<input type="date" value={startDate} onChange={event=>setStartDate(event.target.value)} disabled={saving}/></label><label className="full">Kumpulan Intervensi<select value={groupId} onChange={event=>setGroupId(event.target.value)} disabled={saving||!groups.length}><option value="">Tiada kumpulan</option>{groups.map(group=><option key={group.id} value={group.id}>{group.name} · {group.skillCode} ({group.studentIds.length} murid)</option>)}</select><small className="field-help">{groups.length?`Pilihan ini untuk rekod ${student.subject}. Menukar pilihan akan memindahkan murid daripada kumpulan lama.`:"Belum ada kumpulan. Cipta kumpulan dahulu melalui menu Intervensi."}</small></label>{error&&<div className="full sync-inline-error" role="alert"><AlertCircle size={18}/><span><strong>Kemas kini gagal</strong><small>{error}</small></span></div>}</div></Modal>
}

function DeleteStudentModal({student,close,confirm}:{student:Student;close:()=>void;confirm:(text:string)=>Promise<void>}){const [text,setText]=useState(""),[busy,setBusy]=useState(false),valid=text.trim().toUpperCase()===student.name.trim().toUpperCase();const run=async()=>{if(!valid||busy)return;setBusy(true);try{await confirm(text.trim())}catch{}finally{setBusy(false)}};return <Modal title="Padam Murid?" desc="Semua rekod subjek murid ini akan dipadam daripada sekolah anda." close={close} footer={<><button className="outline" onClick={close} disabled={busy}>Batal</button><button className="primary danger-primary" onClick={()=>void run()} disabled={!valid||busy}><Trash2 size={17}/>{busy?"Memadam...":"Padam Murid"}</button></>}><div className="delete-confirm"><AlertCircle size={27}/><p>Tindakan ini akan memadam <b>{student.name}</b>, penilaian, sasaran dan sejarah intervensinya. Taip nama penuh murid untuk mengesahkan.</p><input autoFocus value={text} onChange={e=>setText(e.target.value)} placeholder={student.name}/></div></Modal>}
...
function InterventionModal({students,groups,selected,initialGroupId,close,save}:{students:Student[];groups:InterventionGroupRecord[];selected:Student|null;initialGroupId:string;close:()=>void;save:(draft:InterventionDraft,requestId:string)=>Promise<number>}){
  const initialMode:InterventionDraft["mode"]=initialGroupId?"group":"individual";
  const initialStudent=selected||students[0];
  const initialGroup=groups.find(group=>group.id===initialGroupId)||groups[0];
  const [mode,setMode]=useState<InterventionDraft["mode"]>(initialMode),[targetId,setTargetId]=useState(initialGroupId||initialStudent?.id||""),[skillCode,setSkillCode]=useState(initialGroup?.skillCode||(initialStudent?.skills["AR 3"]?`KP${initialStudent.skills["AR 3"]}`:"KP1")),[skillName,setSkillName]=useState(initialGroup?.skillName||""),[issue,setIssue]=useState("Lemah membaca perkataan"),[action,setAction]=useState(""),[notes,setNotes]=useState(""),[method,setMethod]=useState(initialMode==="group"?"Kumpulan kecil":"Bimbingan individu"),[start,setStart]=useState(todayIso()),[review,setReview]=useState(todayIso()),[saving,setSaving]=useState(false),[error,setError]=useState("");
  const savingRef=useRef(false),requestId=useRef("");
  if(!requestId.current)requestId.current=globalThis.crypto.randomUUID();
  const targetStudent=students.find(student=>student.id===targetId),targetGroup=groups.find(group=>group.id===targetId);
  const validTarget=mode==="individual"?Boolean(targetStudent):Boolean(targetGroup);
  const switchMode=(next:InterventionDraft["mode"])=>{if(savingRef.current)return;setMode(next);setError("");if(next==="individual"){const student=selected||students[0];setTargetId(student?.id||"");setSkillCode(student?.skills["AR 3"]?`KP${student.skills["AR 3"]}`:"KP1");setSkillName("");setMethod("Bimbingan individu")}else{const group=groups[0];setTargetId(group?.id||"");setSkillCode(group?.skillCode||"KP1");setSkillName(group?.skillName||"");setMethod("Kumpulan kecil")}};
  const changeTarget=(id:string)=>{setTargetId(id);setError("");if(mode==="individual"){const student=students.find(item=>item.id===id);setSkillCode(student?.skills["AR 3"]?`KP${student.skills["AR 3"]}`:"KP1");setSkillName("")}else{const group=groups.find(item=>item.id===id);setSkillCode(group?.skillCode||"KP1");setSkillName(group?.skillName||"")}};
  const submit=async()=>{if(savingRef.current||!validTarget||!action.trim())return;savingRef.current=true;setSaving(true);setError("");try{await save({mode,targetId,skillCode,skillName:skillName.trim(),issue,action:action.trim(),notes:notes.trim(),method,start,review,status:"Sedang dilaksanakan"},requestId.current)}catch(reason){setError(reason instanceof Error?reason.message:"Gagal sync ke Supabase.")}finally{savingRef.current=false;setSaving(false)}};
  if(!students.length)return <Modal title="Rekod Intervensi" desc="Rancang tindakan berdasarkan keperluan murid." close={close} footer={<button className="outline" onClick={close}>Tutup</button>}><div className="empty-state modal-empty"><i><Users size={24}/></i><strong>Tiada murid tersedia</strong><p>Tambah atau selaraskan murid daripada Supabase sebelum merekod intervensi.</p></div></Modal>;
  return <Modal title="Rekod Intervensi" desc="Borang disimpan dalam memori sehingga satu permintaan dihantar apabila anda menekan Simpan." close={close} footer={<><button className="outline" onClick={close} disabled={saving}>Batal</button><button className="primary" disabled={!validTarget||!action.trim()||saving} onClick={()=>void submit()}><Check size={17}/>{saving?"Menyimpan...":error?"Cuba Semula":"Simpan Intervensi"}</button></>}><div className="intervention-form"><div className="intervention-tabs compact" role="tablist"><button role="tab" aria-selected={mode==="individual"} className={mode==="individual"?"active":""} onClick={()=>switchMode("individual")}>Individu</button><button role="tab" aria-selected={mode==="group"} className={mode==="group"?"active":""} onClick={()=>switchMode("group")} disabled={!groups.length}>Berkumpulan</button></div>{mode==="individual"?<><label>Murid<select value={targetId} onChange={event=>changeTarget(event.target.value)} disabled={saving}>{students.map(student=><option key={student.id} value={student.id}>{student.name} · {student.subject}</option>)}</select></label>{targetStudent&&<div className="selected-summary"><b>{initials(targetStudent.name)}</b><span><strong>{targetStudent.name}</strong><small>{targetStudent.className} · Kemahiran semasa {kp(targetStudent.skills["AR 3"])}</small></span></div>}</>:<><label>Kumpulan<select value={targetId} onChange={event=>changeTarget(event.target.value)} disabled={saving}>{groups.map(group=><option key={group.id} value={group.id}>{group.name} ({group.studentIds.length} murid)</option>)}</select></label>{targetGroup&&<div className="selected-summary"><b><Users size={18}/></b><span><strong>{targetGroup.name}</strong><small>{targetGroup.studentIds.length} murid · {targetGroup.skillCode} {targetGroup.skillName}</small></span></div>}</>}<div className="form-grid"><label>Kemahiran<select value={skillCode} onChange={event=>setSkillCode(event.target.value)} disabled={saving}>{skillOptions.filter(value=>value>0).map(value=><option key={value} value={`KP${value}`}>KP{value}</option>)}</select></label><label>Nama kemahiran<input value={skillName} onChange={event=>setSkillName(event.target.value)} placeholder="Contoh: Perkataan KVK" disabled={saving}/></label></div><label>Isu dikenal pasti<select value={issue} onChange={event=>setIssue(event.target.value)} disabled={saving}><option>Keliru huruf</option><option>Lemah membunyikan suku kata</option><option>Lemah menggabung suku kata</option><option>Lemah membaca perkataan</option><option>Lemah menulis</option><option>Tidak mengingat kemahiran</option><option>Kurang fokus</option><option>Kehadiran</option></select></label><label>Intervensi dilaksanakan<textarea rows={3} value={action} onChange={event=>setAction(event.target.value)} placeholder="Contoh: Latihan membina dan membaca perkataan KVK." disabled={saving}/></label><label>Catatan<textarea rows={2} value={notes} onChange={event=>setNotes(event.target.value)} placeholder="Contoh: Menggunakan kad perkataan" disabled={saving}/></label><div className="form-grid"><label>Kaedah<select value={method} onChange={event=>setMethod(event.target.value)} disabled={saving}><option>Bimbingan individu</option><option>Kumpulan kecil</option><option>Latih tubi</option><option>Permainan</option><option>ABM manipulatif</option><option>Bacaan berulang</option></select></label><label>Tarikh mula<input type="date" value={start} onChange={event=>setStart(event.target.value)} disabled={saving}/></label><label>Tarikh semakan<input type="date" value={review} onChange={event=>setReview(event.target.value)} disabled={saving}/></label></div>{error&&<div className="sync-inline-error" role="alert"><AlertCircle size={18}/><span><strong>Gagal sync ke Supabase</strong><small>{error} Data borang masih dikekalkan.</small></span></div>}</div></Modal>;
}

function InterventionGroupModal({students,groups,group,close,save}:{students:Student[];groups:InterventionGroupRecord[];group:InterventionGroupRecord|null;close:()=>void;save:(payload:{groupId?:string;groupName:string;skillCode:string;skillCodes:string[];skillName:string;studentIds:string[]})=>Promise<InterventionGroupRecord>}){
  const initialSubject=students.find(student=>group?.studentIds.includes(student.id))?.subject||students[0]?.subject||"Bahasa Melayu";
  const [name,setName]=useState(group?.name||""),[skillCodes,setSkillCodes]=useState<string[]>(group?.skillCodes?.length?group.skillCodes:[group?.skillCode||"KP1"]),[skillName,setSkillName]=useState(group?.skillName||""),[studentIds,setStudentIds]=useState<string[]>(group?.studentIds||[]),[search,setSearch]=useState(""),[filterYear,setFilterYear]=useState("Semua"),[filterClass,setFilterClass]=useState("Semua"),[filterGroup,setFilterGroup]=useState("Semua"),[filterSubject,setFilterSubject]=useState<Subject>(initialSubject),[saving,setSaving]=useState(false),[error,setError]=useState("");
  const savingRef=useRef(false),classes=[...new Set(students.filter(student=>student.subject===filterSubject).map(student=>student.className))].sort();
  const visible=students.filter(student=>student.subject===filterSubject&&(filterYear==="Semua"||student.year===Number(filterYear))&&(filterClass==="Semua"||student.className===filterClass)&&(filterGroup==="Semua"||(filterGroup==="Tiada"?!groups.some(item=>item.studentIds.includes(student.id)):groups.find(item=>item.id===filterGroup)?.studentIds.includes(student.id)))&&(student.name+" "+student.id).toLowerCase().includes(search.toLowerCase()));
  const toggle=(studentId:string)=>setStudentIds(current=>current.includes(studentId)?current.filter(id=>id!==studentId):[...current,studentId]);
  const toggleSkill=(code:string)=>setSkillCodes(current=>current.includes(code)?current.filter(item=>item!==code):[...current,code].sort((a,b)=>Number(a.slice(2))-Number(b.slice(2))));
  const submit=async()=>{if(savingRef.current||!name.trim()||!skillCodes.length||!studentIds.length)return;savingRef.current=true;setSaving(true);setError("");try{await save({...(group?{groupId:group.id}:{}),groupName:name.trim(),skillCode:skillCodes[0],skillCodes,skillName:skillName.trim(),studentIds});close()}catch(reason){setError(reason instanceof Error?reason.message:"Kumpulan tidak dapat disimpan.")}finally{savingRef.current=false;setSaving(false)}};
  return <Modal title={group?"Edit Kumpulan Intervensi":"Cipta Kumpulan Intervensi"} desc="Pilih satu atau beberapa kemahiran untuk kumpulan yang sama. Keahlian dan kemahiran boleh dikemas kini kemudian." close={close} footer={<><button className="outline" onClick={close} disabled={saving}>Batal</button><button className="primary" disabled={!name.trim()||!skillCodes.length||!studentIds.length||saving} onClick={()=>void submit()}><Check size={17}/>{saving?"Menyimpan...":group?"Simpan Perubahan":"Cipta Kumpulan"}</button></>}><div className="intervention-form"><label>Nama kumpulan<input autoFocus value={name} onChange={event=>setName(event.target.value)} placeholder="Contoh: Kumpulan KVK 1" disabled={saving}/></label><div className="form-grid"><label className="full">Mata pelajaran<select value={filterSubject} onChange={event=>{setFilterSubject(event.target.value as Subject);setStudentIds([]);setFilterClass("Semua")}} disabled={saving}><option>Bahasa Melayu</option><option>Matematik</option></select></label><fieldset className="skill-multi-picker full"><legend>Kemahiran <small>{skillCodes.length} dipilih</small></legend><p>Pilih semua kemahiran yang akan dipelajari oleh kumpulan ini.</p><div>{skillOptions.filter(value=>value>0).map(value=>{const code=`KP${value}`,selected=skillCodes.includes(code);return <label key={code} className={selected?"selected":""}><input type="checkbox" checked={selected} onChange={()=>toggleSkill(code)} disabled={saving}/><span>{code}</span></label>})}</div></fieldset><label className="full">Nama / fokus kumpulan<input value={skillName} onChange={event=>setSkillName(event.target.value)} placeholder="Contoh: Suku kata dan perkataan KVK" disabled={saving}/></label></div><div className="member-filters"><label><Search size={16}/><input value={search} onChange={event=>setSearch(event.target.value)} placeholder="Cari nama atau ID murid"/></label><select value={filterYear} onChange={event=>setFilterYear(event.target.value)}><option value="Semua">Semua Tahun</option>{STUDENT_YEARS.map(value=><option key={value} value={value}>Tahun {value}</option>)}</select><select value={filterClass} onChange={event=>setFilterClass(event.target.value)}><option value="Semua">Semua Kelas</option>{classes.map(value=><option key={value}>{value}</option>)}</select><select value={filterGroup} onChange={event=>setFilterGroup(event.target.value)}><option value="Semua">Semua Kumpulan</option><option value="Tiada">Belum Berkumpulan</option>{groups.filter(item=>item.id!==group?.id).map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></div><div className="member-picker"><header><strong>Pilih ahli</strong><small>{studentIds.length} murid dipilih</small></header>{visible.length?visible.map(student=><label key={student.id} className={studentIds.includes(student.id)?"selected":""}><input type="checkbox" checked={studentIds.includes(student.id)} onChange={()=>toggle(student.id)} disabled={saving}/><b>{initials(student.name)}</b><span><strong>{student.name}</strong><small>Tahun {student.year} · {student.className} · {kp(student.skills["AR 3"])}</small></span></label>):<div className="empty-state modal-empty"><strong>Tiada murid sepadan</strong><p>Ubah carian atau penapis untuk melihat murid lain.</p></div>}</div>{error&&<div className="sync-inline-error" role="alert"><AlertCircle size={18}/><span><strong>Kumpulan tidak dapat disimpan</strong><small>{error} Pilihan ahli masih dikekalkan.</small></span></div>}</div></Modal>;
}
