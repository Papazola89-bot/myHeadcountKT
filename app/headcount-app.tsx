...
export default function HeadcountApp(){
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      if(!appsScriptService){
        if(active){setStudents([]);setSchools([]);setAdmins([]);setSchoolDirectory([]);setTransfers([]);setInterventions([]);setInterventionGroups([]);setAdminInterventions([]);setSyncNotice(null);setSheetStatus("idle");setSaved("Log masuk untuk menyambung Google Sheets")}
        return;
      }
  ...
  const saveIntervention=async(draft:InterventionDraft,requestId:string)=>{
    ...
    announce(result.count>1?`Intervensi berjaya direkodkan kepada ${result.count} orang murid.`:"Intervensi berjaya disimpan ke Google Sheets.");
    return result.count;
  };
