    ...
    <main><header className="topbar"><div><button className="menu-btn" onClick={()=>setMobile(true)}><Menu size={21}/></button><span><small>{role==="guru"?"PORTAL GURU":"PORTAL ADMIN"}</small><strong>{menu.find(x=>x[0]===view)?.[1]||"Tetapan"}</strong></span></div><div>{profile?.role==="ADMIN"&&<span className="role-toggle"><button className={role==="guru"?"active":""} onClick={()=>{setRole("guru");go("dashboard")}}>Guru</button><button className={role==="admin"?"active":""} onClick={()=>{setRole("admin");go("dashboard")}}>Admin</button></span>}<div className={`google-session ${authStatus} ${sheetStatus}`}><span className="connection-state"><i/><b>{authStatus==="signed-in"?"Google telah dilog masuk":authStatus==="loading"?"Memeriksa sesi Google":authStatus==="error"?"Log masuk Google gagal":"Belum log masuk Google"}</b><small>{authStatus==="signed-in"?`${googleEmail} · ${sheetLabel}`:"Log masuk diperlukan untuk Google Sheets"}</small></span>{authStatus==="signed-in"?<button className="google-signout" onClick={signOutGoogle} title="Log keluar Google"><LogOut size={15}/>Log keluar</button>:<div className="google-signin" ref={googleButton}>{!gisReady&&<small>{authStatus==="error"?"Muat semula halaman untuk cuba lagi":"Memuat butang Google..."}</small>}</div>}</div><button className={"bell "+(notificationsOpen?"active":"")} onClick={()=>setNotificationsOpen(open=>!open)} aria-label={notificationsOpen?"Tutup notifikasi":"Buka notifikasi"} aria-expanded={notificationsOpen}><Bell size={20}/>{!notificationsRead&&<i/>}</button>{notificationsOpen&&<><button className="notification-scrim" onClick={()=>setNotificationsOpen(false)} aria-label="Tutup notifikasi"/><aside className="notifications-panel"><header><span><strong>Notifikasi</strong><small>{notificationsRead?"Semua telah dibaca":notifications.length+" notifikasi baharu"}</small></span><button onClick={()=>setNotificationsRead(true)} disabled={notificationsRead}>Tandakan semua dibaca</button></header><div>{notifications.map(({title,detail,time,view:target,Icon,tone})=><button className="notification-item" key={title} onClick={()=>{setNotificationsRead(true);go(target)}}><i className={tone}><Icon size={17}/></i><span><strong>{title}</strong><small>{detail}</small><em>{time}</em></span>{!notificationsRead&&<b/>}</button>)}</div><footer><ShieldCheck size={14}/>Notifikasi sistem myHeadcountKT</footer></aside></>}<button className="profile" onClick={()=>go("settings")}><b className="avatar">{userInitials}</b><span>{userName}</span><ChevronDown size={15}/></button></div></header>
      <div className="page">
        {role==="guru"?<>
          ...
          {view==="dashboard"&&<AdminDashboard go={go} announce={announce}/>} {view==="schools"&&<SchoolsView announce={announce}/>} {view==="headcount"&&<AdminHeadcount announce={announce}/>} {view==="interventions"&&<AdminInterventions/>} {view==="submissions"&&<Submissions announce={announce}/>} {view==="reports"&&<Reports role={role} exportCsv={exportCsv} announce={announce}/>} {view==="users"&&<UsersView announce={announce}/>} {view==="audit"&&<Audit announce={announce}/>} {view==="settings"&&<SettingsView profile={profile} onEdit={()=>profile?setModal("profile"):announce("Log masuk Google untuk mengemas kini profil.")} reset={()=>{persist(seed,students);announce("Data demo dipulihkan.")}}/>}
        </>}
      </div>
    </main>
...
function SchoolsView({announce}:{announce:(message:string)=>void}){return <><Heading eyebrow="PENGURUSAN ORGANISASI" title="Sekolah" desc="Urus sekolah, zon dan guru yang berdaftar dalam sistem."><button className="primary" onClick={()=>announce("Penambahan sekolah akan disediakan selepas modul pengurusan organisasi disambungkan.")}><Plus size={18}/>Tambah Sekolah</button></Heading><section className="card table-card"><Toolbar><button className="outline" onClick={()=>announce("Semua zon sedang dipaparkan.")}><Filter size={16}/>Semua zon</button><button className="outline" onClick={()=>{downloadCsvFile("senarai-sekolah.csv",[["Sekolah","Kod","Zon","Murid","Pencapaian"],...schools.map(s=>s.slice(0,5) as (string|number)[])]);announce("Senarai sekolah telah dieksport.")}}><Download size={16}/>Eksport</button></Toolbar><Table><thead><tr><th>SEKOLAH</th><th>ZON</th><th>GURU</th><th>MURID</th><th>PENCAPAIAN</th><th>STATUS</th><th/></tr></thead><tbody>{schools.map(s=><tr key={s[1]}><td><SchoolCell row={s}/></td><td>{s[2]}</td><td>1 guru</td><td>{s[3]}</td><td><div className="progress-cell"><span><i style={{width:`${s[4]}%`}}/></span><b>{s[4]}%</b></div></td><td><Status text="Aktif"/></td><td><button className="icon" onClick={()=>announce("Butiran sekolah akan dibuka dalam modul pengurusan organisasi.")} aria-label="Pilihan sekolah"><MoreHorizontal size={18}/></button></td></tr>)}</tbody></Table></section></>}

function AdminHeadcount({announce}:{announce:(message:string)=>void}){const heat=[[8,5,3,2,1,0],[5,7,4,2,0,0],[9,8,6,4,2,2],[6,7,4,3,1,1],[3,6,7,5,4,2],[5,5,4,3,2,0]];return <><Heading eyebrow="ANALISIS DAERAH" title="Headcount" desc="Taburan kemahiran murid mengikut sekolah dan zon."><button className="outline" onClick={()=>{downloadCsvFile("headcount-daerah.csv",[["Sekolah","Kod","Zon","Murid","Pencapaian"],...schools.map(s=>s.slice(0,5) as (string|number)[])]);announce("Data headcount daerah telah dieksport.")}}><Download size={17}/>Eksport Data</button></Heading><AdminFilters announce={announce}/><section className="card heatmap-card"><CardHeader title="Taburan Kemahiran Mengikut Sekolah" desc="Warna lebih gelap menunjukkan bilangan murid lebih tinggi"/><div className="heat-wrap"><table><thead><tr><th>SEKOLAH</th><th>KP1–5</th><th>KP6–12</th><th>KP13–19</th><th>KP20–27</th><th>KP28–32</th><th>MENGUASAI</th></tr></thead><tbody>{schools.map((s,r)=><tr key={s[1]}><td><strong>{s[0]}</strong><small>{s[1]}</small></td>{heat[r].map((v,c)=><td key={c}><span data-level={Math.min(Math.ceil(v/2),4)}>{v}</span></td>)}</tr>)}</tbody></table></div><footer className="heat-legend"><span>Rendah</span><i data-level="1"/><i data-level="2"/><i data-level="3"/><i data-level="4"/><span>Tinggi</span></footer></section></>}
