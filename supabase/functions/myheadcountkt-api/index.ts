...
function normalizeSkills(value: unknown) {
  ...
  const skills = [...new Set(source.filter((item) => text(item)).map(normalizeSkill))]
    .sort((left, right) => skillNumber(left) - skillNumber(right));
  if (!skills.length) fail("VALIDATION_ERROR", "Pilih sekurang-kurangnya satu kemahiran.");
  return skills;
}
...
async function dispatch(action: string, input: Row, actor: Actor | null): Promise<unknown> {
  ...
  if (action === "importAnalysis") {
    assertGuru(actor);
    const sourceRows = Array.isArray(input.rows) ? input.rows : [];
    if (!sourceRows.length || sourceRows.length > 1000) fail("VALIDATION_ERROR", "Fail mesti mengandungi antara 1 hingga 1000 rekod kemahiran.");
    const cycle = normalizeCycle(input.cycle);
    if (!/^AR [1-3]$/.test(cycle)) fail("INVALID_SCREENING", "Pilih Saringan 1, 2 atau 3.");
    const yearData = Number(input.tahun_data || input.year || new Date().getFullYear());
    if (yearData < 2020 || yearData > 2100) fail("INVALID_DATA_YEAR", "Tahun data tidak sah.");
    const now = new Date().toISOString();
    const uniqueRows = new Map<string, Row>();
    for (const raw of sourceRows) {
      const row = raw as Row, nama = required(row.name || row.nama, "Nama murid", 300).replace(/\s+/g, " "), tahun = Number(row.year || row.tahun), subject = normalizeSubject(row.subject || row.subjek), skillCode = normalizeSkill(row.skillCode || row.skill_code);
      if (tahun < 2 || tahun > 6) fail("INVALID_YEAR", `Darjah ${nama} mestilah Tahun 2 hingga Tahun 6.`);
      uniqueRows.set(`${upper(nama)}|${subject}`, { nama, tahun, kelas: text(row.className || row.kelas) || `Tahun ${tahun}`, subject, skillCode });
    }
    const existingResult = await supabase.from("students").select("*").eq("school_id", actor.school_id);
    if (existingResult.error) fail("DATABASE_ERROR", existingResult.error.message, 500);
    const existingMap = new Map((existingResult.data || []).map(row => [`${upper(row.nama)}|${row.subject}`, row]));
    const studentRows = [...uniqueRows.values()].map(row => {
      const existing = existingMap.get(`${upper(row.nama)}|${row.subject}`);
      return { student_id: existing?.student_id || `ST-${uuid()}`, school_id: actor.school_id, nama: row.nama, tahun: row.tahun, kelas: existing?.kelas || row.kelas, tarikh_mula: existing?.tarikh_mula || now.slice(0, 10), subject: row.subject, status: "Aktif", created_at: existing?.created_at || now, updated_at: now };
    });
    const savedStudents = await supabase.from("students").upsert(studentRows, { onConflict: "student_id" }).select("*");
    if (savedStudents.error) fail("DATABASE_ERROR", savedStudents.error.message, 500);
    const savedMap = new Map((savedStudents.data || []).map(row => [`${upper(row.nama)}|${row.subject}`, row]));
    const studentIds = (savedStudents.data || []).map(row => row.student_id);
    const assessmentResult = await supabase.from("assessments").select("*").in("student_id", studentIds).eq("tahun_data", yearData).eq("cycle", cycle);
    if (assessmentResult.error) fail("DATABASE_ERROR", assessmentResult.error.message, 500);
    const assessmentMap = new Map((assessmentResult.data || []).map(row => [`${row.student_id}|${row.subject}`, row]));
    const assessmentRows = [...uniqueRows.values()].map(row => {
      const student = savedMap.get(`${upper(row.nama)}|${row.subject}`), existing = assessmentMap.get(`${student.student_id}|${row.subject}`);
      return { assessment_id: existing?.assessment_id || uuid(), student_id: student.student_id, subject: row.subject, tahun_data: yearData, cycle, skill_code: row.skillCode, tarikh: now, teacher_id: actor.user_id, created_at: existing?.created_at || now, updated_at: now };
    });
    const savedAssessments = await supabase.from("assessments").upsert(assessmentRows, { onConflict: "assessment_id" });
    if (savedAssessments.error) fail("DATABASE_ERROR", savedAssessments.error.message, 500);
    const pupilCount = new Set([...uniqueRows.values()].map(row => upper(row.nama))).size;
    await audit(actor, "IMPORT_ANALYSIS", null, { cycle, tahun_data: yearData, pupil_count: pupilCount, record_count: assessmentRows.length });
    return { saved: true, cycle, pupil_count: pupilCount, record_count: assessmentRows.length, students: await getStudents(actor) };
  }
  if (action === "deleteStudent") {
    assertGuru(actor);
    const studentId = required(input.studentId || input.student_id, "studentId"), student = await ownedStudent(studentId, actor);
    if (upper(input.confirmation) !== upper(student.nama)) fail("CONFIRMATION_REQUIRED", "Taip nama penuh murid untuk mengesahkan pemadaman.");
    ...
    const ids = (related.data || []).map(row => row.student_id);
    const groups = await supabase.from("intervention_groups").select("*").eq("school_id", actor.school_id);
    ...
    for (const group of groups.data || []) {
      ...
      const nextMembers = members.filter(id => !ids.includes(id));
      if (nextMembers.length !== members.length) {
        const updated = await supabase.from("intervention_groups").update({ student_ids: nextMembers, updated_at: new Date().toISOString() }).eq("group_id", group.group_id).eq("school_id", actor.school_id);
        if (updated.error) fail("DATABASE_ERROR", updated.error.message, 500);
      }
    }
    const removed = await supabase.from("students").delete().in("student_id", ids).eq("school_id", actor.school_id);
  ...
  if (action === "saveInterventionGroup") { assertGuru(actor); const groupId = text(input.groupId || input.group_id) || `GRP-${uuid()}`, ids = [...new Set((input.studentIds || input.student_ids || []).map(String))], skillCodes = normalizeSkills(input.skillCodes || input.skill_codes || input.skillCode || input.skill_code); if (!ids.length) fail("VALIDATION_ERROR", "Pilih sekurang-kurangnya seorang murid."); for (const id of ids) await ownedStudent(id, actor); const existing = await one("intervention_groups", "group_id", groupId); if (existing && existing.school_id !== actor.school_id) fail("GROUP_ACCESS_DENIED", "Kumpulan ini bukan di bawah sekolah anda.", 403); const record = { group_id: groupId, school_id: actor.school_id, group_name: required(input.groupName || input.group_name, "Nama kumpulan", 200), skill_code: skillCodes[0], skill_codes: skillCodes, skill_name: text(input.skillName || input.skill_name), student_ids: ids, created_by: existing?.created_by || actor.user_id, created_at: existing?.created_at || new Date().toISOString(), updated_at: new Date().toISOString() }; const result = await supabase.from("intervention_groups").upsert(record).select("*").single(); if (result.error) fail("DATABASE_ERROR", result.error.message, 500); await audit(actor, "SAVE_INTERVENTION_GROUP", existing, result.data); return { ...result.data, student_ids_json: result.data.student_ids }; }
